...
model_rnvp = RNVP(1, 4)
out = train_apply(model=model_rnvp, n_train=10, dataset='MNIST', epochs=10, batch_size=128,
...
test_dataset = torchvision.datasets.MNIST('/home/space/datasets', train=False, transform=torchvision.transforms.ToTensor(), download=True)
...
plt.subplot(1, 3, 2)
plt.title('Normal distributed image that will pass through the reversed model')
...
directory = '/home/pml_07/MLP'
file_name = '../../../../Downloads/reconstructed.png'
path = os.path.join(directory, file_name)
plt.savefig(path)
